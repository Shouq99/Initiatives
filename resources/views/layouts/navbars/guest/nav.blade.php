...
<nav class="navbar navbar-expand-lg position-absolute top-0 z-index-3 my-3 {{ (Request::is('static-sign-up') ? 'w-100 shadow-none  navbar-transparent mt-4' : 'blur blur-rounded shadow py-2 start-0 end-0 mx4') }} ">
  <div class="container-fluid {{ (Request::is('static-sign-up') ? 'container' : 'container-fluid') }}">
    <a class="navbar-brand font-weight-bolder text-secondrycolor ms-lg-0 ms-3 {{ (Request::is('static-sign-up') ? 'text-secondrycolor' : '') }}" href="{{ url('dashboard') }}">
      بانوراما القصيم داشبورد
    </a>
    ...
    <div class="collapse navbar-collapse" id="navigation">
      <ul class="navbar-nav mx-auto">
        ...
        <li class="nav-item">
          <a class="nav-link me-2" href="{{ auth()->user() ? url('static-sign-up') : url('register') }}">
            <i class="fas fa-user-circle opacity-6 me-1 {{ (Request::is('static-sign-up') ? '' : 'text-primarydarkcolor') }}"></i>
            Sign Up
          </a>
        </li>
        <li class="nav-item">
          <a class="nav-link me-2" href="{{ auth()->user() ? url('static-sign-in') : url('login') }}">
            <i class="fas fa-key opacity-6 me-1 {{ (Request::is('static-sign-up') ? '' : 'text-primarydarkcolor') }}"></i>
            Sign In
          </a>
        </li>
      </ul>
      <ul class="navbar-nav d-lg-block d-none">
        <li class="nav-item">
          <a class="navbar-brand me-5" href="#"><img src="{{asset('assets/img/logo.png')}}" width="50px" height="60px"></a>
        </li>
      </ul>
    </div>
  </div>
